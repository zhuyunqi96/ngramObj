# coding=utf-8
# (yunqi) the following codes are based on huggingface/transformers. 
...
from transformers.generation_utils import (
    GenerationMixin, StoppingCriteriaList
)
...
# from beam_search_custom import beamSearchCustom, decode_labels, ngram_score
...
class customSeqModelOutput(ModelOutput):
    ...
    hierReSimL: Optional[torch.FloatTensor] = None
    goldsumSimL: Optional[torch.FloatTensor] = None
    concSynSimL: Optional[torch.FloatTensor] = None
    hierSentSim: Optional[Tuple[torch.FloatTensor]] = None
    concSynLabel: Optional[torch.FloatTensor] = None


class customBaseModelOutput(ModelOutput):
    last_hidden_state: torch.FloatTensor = None
    past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
...
class customEncoderModelOutput(ModelOutput):
    last_hidden_state: torch.FloatTensor = None
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None
...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        attn_weights = torch_F.softmax(attn_weights, dim=-1)
        ...
        attn_probs = torch_F.dropout(attn_weights, p=self.dropout, training=self.training)
...
class BartEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        #     hidden_states = hidden_states + mlp_hidden # mlp custom
...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([BartEncoderLayer(config, layer_idx) for layer_idx in range(config.encoder_layers)])
    ...
    def forward(
        ...
        return customEncoderModelOutput(
...
class BartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig, max_target_length: int):
        super().__init__(config)
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, max_target_length: int):
        super().__init__(config)
        self.model = BartModel(config, max_target_length)
    ...
    def decode_labels(self, labels_to_decode):
        labels_np = labels_to_decode.cpu()
        labels_np = np.where(labels_np != -100, labels_np, self.tokenizer.pad_token_id)
        return self.tokenizer.decode(labels_np, skip_special_tokens=True)
    ...
    def forward(
        ...
        if USE_NgramsLoss and self.training:
            ...
            softmaxed_lm_logits = torch_F.softmax(lm_logits, dim=-1)

            for b_i in range(btz):
                oneGram_loss_i = 1
                twoGram_loss_i = 1 # init n-gram loss == 1, because we wanna reward matched twograms when we find all tokens of the n-gram has max prob compare to the other tokens
                triGram_loss_i = 1
                fourGram_loss_i = 1
                fiveGram_loss_i = 1

                seq_size = (labels[b_i] < 0).nonzero(as_tuple=True)[0].cpu().numpy() # find index of the end of this gold summary
                seq_size = tgt_size if seq_size.shape[0] == 0 else seq_size[0]
                
                goldsum_i = labels[b_i, :seq_size] # not including padding

                summary_prob = softmaxed_lm_logits[b_i] # softmax embeddings, index b_i summary
                ...
                if USE_BoN:
                    ...
                    for j in range(seq_size - 1):
                        two_grams[(goldsum_i[j], goldsum_i[j+1])] += 1
                    ...
                    match_gram_1 = summary_prob[0:seq_size-1, gram_1]
                    match_gram_2 = summary_prob[0+1:seq_size, gram_2]
                    match_gram = match_gram_1 * match_gram_2
                    match_gram = torch.sum(match_gram, dim = 0).view(-1,1)

                    gram_count = torch.Tensor(gram_count).to(summary_prob.device).view(-1,1)
                    match_gram_cat = torch.cat([match_gram, gram_count],dim = -1)
                    match_gram = torch.min(match_gram_cat, dim = -1)[0]
                ...
                if USE_p2loss: # from https://github.com/ictnlp/GS4NMT/blob/master/models/losser.py
                    ...
                    sum_two_gram = 0 
                    two_gram = Counter()
                    for s_i in range(seq_size - 1):
                        # two_gram[(goldsum_i[s_i], goldsum_i[s_i+1])] += 1 # not working
                        two_gram[tuple(goldsum_i[s_i:s_i+2].tolist())] += 1
                    pred_two_gram = Counter()
                    for s_i in range(seq_size - 1):
                        # pred_two_gram[(cand_indices[s_i], cand_indices[s_i+1])] = prob_values[s_i] * prob_values[s_i+1] # not working
                        pred_two_gram[tuple(cand_indices[s_i:s_i+2].tolist())] = prob_values[s_i] * prob_values[s_i+1]
                        sum_two_gram += prob_values[s_i] * prob_values[s_i+1]
                        
                    for ngram in two_gram:
                        if ngram in pred_two_gram:
                            if two_gram[ngram] > pred_two_gram[ngram]:
                                match_two_gram += pred_two_gram[ngram]
                            else:
                                match_two_gram += two_gram[ngram]
                ...
                for s_i in range(seq_size):
                    if USE_Ngrams_reward:
                        ...
                        if _value is None:
                            _value = []
                        # if isMaxProb[s_i]:
                        if torch.all(cand_indices[s_i]==goldsum_i[s_i]):
                            _value.append(prob_values[s_i])
                        oneGrams[_key] = _value
                        
                        if s_i <= seq_size - 2:
                            _key = '_'.join([str(i) for i in goldsum_i[s_i:s_i+2].tolist()])
                            _value = twoGrams.get(_key)
                            if _value is None:
                                _value = []
                            # if torch.all(isMaxProb[s_i:s_i+2]):
                            if torch.all(cand_indices[s_i:s_i+2]==goldsum_i[s_i:s_i+2]):
                                _value.append(torch.prod(prob_values[s_i:s_i+2]))
                            twoGrams[_key] = _value # example of key: 0_1, meaning first token of this two gram is 0, second token is 1

                        if s_i <= seq_size - 3:
                            _key = '_'.join([str(i) for i in goldsum_i[s_i:s_i+3].tolist()])
                            _value = triGrams.get(_key)
                            if _value is None:
                                _value = []
                            # if torch.all(isMaxProb[s_i:s_i+3]):
                            if torch.all(cand_indices[s_i:s_i+3]==goldsum_i[s_i:s_i+3]):
                                _value.append(torch.prod(prob_values[s_i:s_i+3]))
                            triGrams[_key] = _value
                        if s_i <= seq_size - 4:
                            _key = '_'.join([str(i) for i in goldsum_i[s_i:s_i+4].tolist()])
                            _value = fourGrams.get(_key)
                            if _value is None:
                                _value = []
                            # if torch.all(isMaxProb[s_i:s_i+4]):
                            if torch.all(cand_indices[s_i:s_i+4]==goldsum_i[s_i:s_i+4]):
                        ...
                        if s_i <= seq_size - 5:
                            ...
                            if _value is None:
                                _value = []
                            # if torch.all(isMaxProb[s_i:s_i+5]):
                            if torch.all(cand_indices[s_i:s_i+5]==goldsum_i[s_i:s_i+5]):
                ...
                if USE_2GramLoss and seq_size > 1:
                    ...
                    for key, value in twoGrams.items():
                        ...
                        for similarity in value:
                            twoGram_loss_i -= (each_2Gram_loss / num_of_2Grams * similarity)
                ...
                if USE_5GramLoss and seq_size > 4:
                    ...
                    for key, value in fiveGrams.items():
                        ...
                        for similarity in value:
                            fiveGram_loss_i -= (each_5Gram_loss / num_of_5Grams * similarity)

                oneGram_loss += oneGram_loss_i
                twoGrams_loss += twoGram_loss_i # sum up
                triGrams_loss += triGram_loss_i
                fourGrams_loss += fourGram_loss_i
                fiveGrams_loss += fiveGram_loss_i
            
            oneGram_loss /= btz
            twoGrams_loss /= btz # get avg
            triGrams_loss /= btz
            fourGrams_loss /= btz
            fiveGrams_loss /= btz
            BoN_NAT_loss /= btz
            ...
            try:
                self.loss_Ngrams['l0'] += oneGram_loss.item()
            except:
                self.loss_Ngrams['l0'] += oneGram_loss
            try:
                self.loss_Ngrams['l2'] += twoGrams_loss.item()
            except:
                self.loss_Ngrams['l2'] += twoGrams_loss
            try:
                self.loss_Ngrams['l3'] += triGrams_loss.item()
            except:
                self.loss_Ngrams['l3'] += triGrams_loss
            try:
                self.loss_Ngrams['l4'] += BoN_NAT_loss.item()
            except:
                self.loss_Ngrams['l4'] += BoN_NAT_loss
            try:
                self.loss_Ngrams['l5'] += fourGrams_loss.item()
            except:
                self.loss_Ngrams['l5'] += fourGrams_loss
            try:
                self.loss_Ngrams['l6'] += fiveGrams_loss.item()
            except:
            ...
            if self.train_count % log_margin == 0 and self.train_count > 0:
                if self.loss_Ngrams['c'] >= 1:
                    ...
                    loss_twoGrams_avg = round(self.loss_Ngrams['l2']/self.loss_Ngrams['c'], 4)
                    loss_triGrams_avg = round(self.loss_Ngrams['l3']/self.loss_Ngrams['c'], 8)
                    BoN_NAT_avg = round(self.loss_Ngrams['l4']/self.loss_Ngrams['c'], 4)
                    loss_fourGrams_avg = round(self.loss_Ngrams['l5']/self.loss_Ngrams['c'], 8)
                    loss_fiveGrams_avg = round(self.loss_Ngrams['l6']/self.loss_Ngrams['c'], 8)
                    logger.warning(f' custLoss n-grams {loss_avg} {loss_oneGrams_avg} {loss_twoGrams_avg} {loss_triGrams_avg} {BoN_NAT_avg} {loss_fourGrams_avg} {loss_fiveGrams_avg}')
                self.loss_Ngrams['l1'], self.loss_Ngrams['l0'], self.loss_Ngrams['l2'], self.loss_Ngrams['l3'], self.loss_Ngrams['l4'], self.loss_Ngrams['l5'], self.loss_Ngrams['l6'], self.loss_Ngrams['c'] = 0, 0, 0, 0, 0, 0, 0, 0

            masked_lm_loss += oneGram_loss
            masked_lm_loss += twoGrams_loss
            masked_lm_loss += triGrams_loss
            masked_lm_loss += fourGrams_loss
            masked_lm_loss += fiveGrams_loss
            masked_lm_loss += BoN_NAT_loss

        self.train_count += 1

        if not return_dict:
            output = (lm_logits,) + outputs[1:] # lm_logits
